...
public class CatwalkStairsBlock extends Block {
    ...
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(Properties.FACING, ConveyorProperties.LEFT, ConveyorProperties.RIGHT);
    }
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction facing, BlockState newState, WorldAccess world, BlockPos blockPos, BlockPos posFrom) {
        newState = state
                .with(ConveyorProperties.RIGHT, this.isAdjacentBlockOfMyType(world, blockPos, state.get(Properties.FACING).rotateYClockwise()))
                .with(ConveyorProperties.LEFT, this.isAdjacentBlockOfMyType(world, blockPos, state.get(Properties.FACING).rotateYCounterclockwise()));
